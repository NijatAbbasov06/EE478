...
class AprilTagLocalizationNode:
    ...
    def tag_callback(self, msg):
        ...
        if gate_index < len(self.gate_defs):
            ...
            quat_before = [rel_orientation.x, rel_orientation.y, rel_orientation.z, rel_orientation.w]
            quat = self.quat_filter.ensure_continuity(quat_before)
            ...
            if(base_world_pose[2]<6):
            # Publish the base world pose
                self.publish_base_pose(base_world_pose, camera_world_rot, msg.header.stamp)
            else:
    ...
    def publish_base_pose(self, base_world_pos, camera_world_rot, timestamp):
        ...
        pose_msg.pose.position.x = base_world_pos[0]
        pose_msg.pose.position.y = base_world_pos[1]
        pose_msg.pose.position.z = base_world_pos[2]
        ...
        camera_rot_matrix = camera_world_rot.as_matrix()
        ...
        base_rot_matrix = camera_rot_matrix @ self.transform_matrix
        base_rotation = Rotation.from_matrix(base_rot_matrix)
        base_quat = base_rotation.as_quat()  # [x, y, z, w]
